                ...
                return (
                  <div key={index} className="space-y-3 border-t border-stone-200 pt-3 first:border-t-0 first:pt-0">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      {type === "tempmail_lol" || type === "moemail" || type === "duckmail" || type === "gptmail" || type === "yyds_mail" || type === "luckmail" ? (
                        <div className="space-y-2">
                      ...
                      {type === "luckmail" ? (
                        <>
                          <div className="space-y-2">
                            ...
                            <Input value={String(provider.project_code || "openai")} onChange={(event) => updateProvider(index, { project_code: event.target.value })} placeholder="openai" className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
                          </div>
                          <div className="space-y-2">
                            <label className="text-sm text-stone-700">Email Type</label>
                            <Input value={String(provider.email_type || "")} onChange={(event) => updateProvider(index, { email_type: event.target.value })} placeholder="ms_graph" className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
                          </div>
                          <div className="space-y-2">
                            <label className="text-sm text-stone-700">Variant Mode</label>
                            <Input value={String(provider.variant_mode || "")} onChange={(event) => updateProvider(index, { variant_mode: event.target.value })} placeholder="dot / plus / mixed / all" className="h-10 rounded-xl border-stone-200 bg-white" disabled={config.enabled} />
                          </div>
                          <div className="space-y-2">
                          ...
                          <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700 md:col-span-2">
                            LuckMail 使用购买邮箱接口；同一邮箱失败后按重试次数继续尝试，达到上限后才结束该任务。
                          </div>
                        </>
                      ) : null}
                      {type === "outlook_token" ? (
                    ...
                    {type === "cloudmail_gen" || type === "tempmail_lol" || type === "cloudflare_temp_email" || type === "moemail" || type === "inbucket" || type === "yyds_mail" || type === "ddg_mail" || type === "luckmail" ? (
                      <div className="space-y-2">
                        ...
                        <Textarea value={domains} onChange={(event) => updateProvider(index, { domain: event.target.value.split(/[\n,]/).map((item) => item.trim()) })} placeholder={type === "cloudmail_gen" ? "每行一个域名，必填" : type === "inbucket" ? "每行一个基础域名，系统会自动生成随机子域名" : type === "moemail" ? "每行一个域名" : "每行一个域名，留空则使用服务默认域名"} className="min-h-20 rounded-xl border-stone-200 bg-white font-mono text-xs" disabled={config.enabled} />
                      </div>
                    ) : null}
                    {type === "cloudmail_gen" ? (
                      <div className="space-y-2">
